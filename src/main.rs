...
fn main() {
    ...
    let mut client_threads:Vec<_> = Vec::new();
    client_threads.push(mpsc::channel());
    client_threads.push(mpsc::channel());
    ...
    thread::spawn(move || {
        ...
        let mut resp = client_threads[0].1.recv().unwrap();
        print!("Client 0: {}\n",resp);
    ...
    print!("Server about to send: {}",resp);
    let mut tmp = &client_threads[(requester as usize)];
    tmp.0.send(resp).unwrap();
}
